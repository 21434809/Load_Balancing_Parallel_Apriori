...
class UnifiedDataLoader:
    ...
    def _build_tid_structure(self, verbose: bool = True):
        """Build TID structure for WDPA - WITH CACHING!"""
        import os
        import hashlib
        from src.tid import TID
        
        # Create cache filename based on dataset parameters  
        cache_key = f"{self.sample_size}_{self.max_items}_{self.random_seed}"
        cache_hash = hashlib.md5(cache_key.encode()).hexdigest()[:8]
        cache_dir = 'results/tid_cache'
        os.makedirs(cache_dir, exist_ok=True)
        cache_file = f'{cache_dir}/tid_{self.sample_size}_{cache_hash}.pkl'
        
        # Try to load from cache
        if os.path.exists(cache_file):
            if verbose:
                print(f"  Loading cached TID from: {cache_file}")
            try:
                tid = TID.load(cache_file)
                if verbose:
                    print(f"  Cached TID loaded: {tid.total_transactions:,} transactions, {len(tid.item_to_tids):,} items")
                return tid
            except Exception as e:
                if verbose:
                    print(f"  Cache load failed: {e}, rebuilding...")
        
        # Build TID from scratch
        if verbose:
            print(f"  Building new TID structure...")
        ...
        try:
            tid.save(cache_file)
            if verbose:
                print(f"  TID cached to: {cache_file}")
        except Exception as e:
            if verbose:
